...
from services.face_recognition import load_h5_embeddings, find_matching_face, update_dataset, delete_user_from_h5
...
@api_route_bp.route('/api/login', methods=['POST','OPTIONS'])
def login():
    if request.method == 'OPTIONS':
    # ⏳ Preflight CORS check butuh balasan 200 + header CORS
    	return '', 200
    try:
        data = request.get_json()
        email = data.get("email")
...
@api_route_bp.route('/api/getattendance', methods=['GET','OPTIONS'])
def get_attendance():
    ...
    try:
        ...
        if conn is not None:  # <--- WAJIB PAKAI PENGECEKAN!
            conn.close()

@api_route_bp.route('/api/getattendanceMonth', methods=['GET','OPTIONS'])
